...
/*
 ...
 Derived from Network library Client.java
 ...
 */
public class SshClient implements Client, Runnable {
   ...
   */
  SshClient(PApplet parent, String host, int port, String user, String password) {
    this.parent = parent;
    this.host = host;
    this.port = port;
    ...
    try
    {
      ...
      session.connect(2000); // connection timeout 2 seconds for local network
  ...
  void dispose() {
    ...
    if (channel.isConnected()) {
      ...
      channel.disconnect();
      session.disconnect();
      System.out.println("SSH Client Finished.\n");
    }
  }
